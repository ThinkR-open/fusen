...
# Test gui ----
dummygui <- tempfile(pattern = "dummygui")
the_dir <- dirname(dummygui)
the_path <- basename(dummygui)

withr::with_dir(the_dir, {
  test_that("Can create in a subdirectory", {
    expect_error(create_fusen_gui(the_path, name = "teaching"), regexp = NA)
    expect_true(dir.exists(dummygui))
  })
})

unlink(dummypackage, recursive = TRUE)
unlink(dummysubdir, recursive = TRUE)
unlink(dummygui, recursive = TRUE)
