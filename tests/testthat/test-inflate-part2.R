...
usethis::with_project(dummypackage, {
  ...
  test_that("Deal with noRd but examples", {
    ...
    expect_error(rcmdcheck::rcmdcheck(
      dummypackage,
      quiet = TRUE,
      args = c("--no-manual")
    ))
  ...
  test_that("inflate() output no error", {
    ...
    if (!interactive()) {
      ...
      check_lines <- readLines(
        file.path(
          checkdir,
          paste0(basename(dummypackage), ".Rcheck"),
          "00check.log"
        )
      )
      expect_equal(check_lines[length(check_lines)], "Status: OK")
...
usethis::with_project(dummypackage, {
  suppressMessages(
    inflate(
      pkg = dummypackage,
      flat_file = flat_file,
      vignette_name = c("Super title" = "01-Super Slug"),
      check = FALSE,
      open_vignette = FALSE
    )
  )
