...
Indeed, this {fusen} package is a real-world example of {fusen} use as it was itself created from the `"dev_history.Rmd"` templates availables in `"dev/"` folder in its GitHub repository.
...
_Note that this package relies on {parsermd}, which is also under development and not currently released on CRAN._
...
- Follow the `"dev/dev_history.Rmd"` template to write your documentation and build your functions and test your examples.  
  ...
  - Chunk named `example` gets the code for examples of using the function. This will be used for function `@examples` and will be kept for the vignette. 
    + As chunk names should be unique in the future vignette, you can add numbers like `example-1`, `example-2`, ...
