...
#'
#' @examples
add_dev_history <- function(pkg = ".", overwrite = FALSE, open = TRUE) {
  old <- setwd(pkg)
  on.exit(setwd(old))
  ...
  if (file.exists(dev_path) & overwrite == FALSE) {
    n < length(list.files(pattern = "^dev_history"))
    dev_path <- file.path(pkg_path, paste0("dev_history_", n + 1, ".Rmd"))
    message("dev_history.Rmd already exists. New dev file is renamed '",
  ...
  usethis::use_build_ignore(basename(dev_path))
...
#'
#' @examples
inflate <- function(pkg = ".", rmd = "dev_history.Rmd", name = "exploration", check = TRUE) {
  old <- setwd(pkg)
  on.exit(setwd(old))
  ...
  # _Get examples
  fun_code <- add_fun_code_examples(fun_code)
...
add_fun_code_examples <- function(fun_code) {
...
create_vignette <- function(parsed_tbl, pkg_path, name) {
  ...
  if (!dir.exists(vignette_dir)) {dir.create(vignette_dir)}
  ...
  vignette_file <- file.path(vignette_dir, paste0(name, ".Rmd"))
