...
inflate <- function(pkg = ".", rmd = "dev_history.Rmd", check = TRUE) {
  ...
  if (nrow(rmd_fun) != 0) {
    ...
    fun_names <- unlist(lapply(seq_along(nrow(rmd_fun)), function(x) parse_fun(rmd_fun[x,])))
    ...
    parsed_tbl$fun <- NA_character_
    # Function name
    parsed_tbl[["fun"]][which_parsed_fun] <- fun_names
        
    pkg_filled <- lapply(na.omit(unique(parsed_tbl[["sec_title"]])), function(x) {
      group <- which(parsed_tbl[["sec_title"]] == x)
      parsed_tbl[group,] <- tidyr::fill(parsed_tbl[group,], fun)
    }) %>% 
      do.call("rbind", .)
    parsed_tbl[["fun"]][pkg_filled[["order"]]] <- pkg_filled[["fun"]]
  }
