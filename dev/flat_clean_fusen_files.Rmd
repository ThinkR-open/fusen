...
test_that("config file is correctly built after half register", {
  ...
  if (file.exists("config_fusen_register.yaml")) {
    ...
    out_expected <- yaml::read_yaml(here::here("tests/testthat/config_fusen_register_inflate.yaml"))
  }
...
test_that("df_to_config works", {
  withr::with_dir(dir_tmp, {
    withr::with_options(list(fusen.config_file = config_file_path), {
      ...
      # inflate_parameters <- formals(fusen::inflate)
      # inflate_parameters <- inflate_parameters[-which(names(inflate_parameters) == "...")]
      # inflate_parameters[["flat_file"]] <- "toto.Rmd"
    ...
    expect_equal(names(all_keep$keep), c("path", "state", "R", "tests", "vignettes")) #, "inflate"
...
test_that("df_to_config works with files having no content", {
  withr::with_options(list(fusen.config_file = config_file_path), {
    withr::with_dir(dir_tmp, {
      ...
      expect_error(config_file_out <- df_to_config(all_files),#, inflate_parameters = inflate_parameters),
                   regexp = "zaza.R")
    })
  })
})
