---
title: "flat_map_the_package.Rmd empty"
...
```{r function-map_the_package}
#' Map information of a 'fusen' built package for developers
#'
#' @param yaml_fusen_file The path to the yaml file
#' to get the map from
...
#'
#' @examples
map_the_package <- function(yaml_fusen_file, emoji = TRUE) {
  if (missing(yaml_fusen_file)) {
    yaml_fusen_file <- getOption("fusen_config_file", "dev/config_fusen.yaml")
  }
  print(normalize_path_winslash(yaml_fusen_file))
...
draw_the_map <- function(x) {
  ...
  cat(unlist(to_tree(map)))
}
```

```{r examples-map_the_package, eval=FALSE}
#' \dontrun{
...
fusen::inflate(
  flat_file = "dev/flat_map_the_package.Rmd",
