...
add_flat_template <- function(
  ...
  if (template == "dev_history") {
    dev_path <- file.path(dev_dir, "0-dev_history.Rmd")
  } else {
    ...
    dev_path <- file.path(dev_dir, flat_name) #"dev_history.Rmd")
    
    if (file.exists(dev_path) & overwrite == FALSE) {
      n <- length(list.files(dev_dir, pattern = "^flat_.*[.]Rmd"))
      dev_path <- file.path(dev_dir, paste0(file_path_sans_ext(flat_name), "_", n + 1, ".Rmd"))
      message(
...
test_that("add_flat_template adds dev_history.Rmd and co.", {
  dev_path <- expect_error(add_flat_template(pkg = dummypackage, open = FALSE), 
               regexp = NA)
  expect_true(file.exists(dev_path))
  expect_true(file.exists(file.path(dirname(dev_path), "0-dev_history.Rmd")))
...
# Add
dev_path <- add_flat_template(pkg = dummypackage2, open = FALSE)
...
# TODO
stop("create test for dev_history template!")
...
for (template in all_templates) {
  ...
  # Add
  dev_path <- add_flat_template(pkg = dummypackage4, template = template, open = FALSE)
  # Change lines asking for pkg name
...
dummypackage5 <- tempfile(pattern = "dummy4")
dir.create(dummypackage5)
# Add
test_that("Other flat_name works", {
  dev_path <- expect_error(
    add_flat_template(pkg = dummypackage4, flat_name = "hello", open = FALSE),
    regexp = NA)
  expect_true(file.exists(file.path(dummypackage4, "dev/flat_hello.Rmd")))
})
# Delete dummy package
unlink(dummypackage5, recursive = TRUE)
...
fusen::inflate(rmd = "dev/dev_history_flat.Rmd")
```
