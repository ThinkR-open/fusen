...
inflate_all <- function(pkg = ".", document = TRUE, check = TRUE, open_vignette = FALSE, overwrite = TRUE, clean = TRUE, ...) {
  ...
  if (stop_after_infos) {
    cli::cli_abort(
      c(
        " {.fn inflate_all} requires a configuration file to work properly.",
        ...
        "\nThis error is common if you were using {.pkg fusen} prior to v0.5.1. Read `vignette('{.vignette inflate-all-your-flat-files}', package = 'fusen')` for more information."
      )
    )
    return(NULL)
  }
